...
# default XLS file name for deploy condifuration
...
# full path to the remote destination folder where BlueCare stores it's configuration files
if ( $ApplyConfiguration ) { $g_CfgDestinationFolder = $g_DataSettings.Configuration | % { $_.g_CfgDestinationFolder.value }
...
# If you want to store logs locally at script path (separated from configuration source) then uncomment lines below
...
function Start-ConfigurationDeploy { param( [parameter(Position=0,Mandatory=$true)]$xData,[int]$Throttle = 10)
Begin {
Write-Host ""
...
Function Get-RunspaceData { [cmdletbinding()]param( $dataVariable, [switch]$Wait )
Do {
    $more = $false
...
# Name of the source file from execel, regardless of whether the file exists in data variable or not
$cfgSourceFileName = $CSV_Row."$CSV_ColumnName"
# Does xls document had any data in server+column cell? If not, then there wasn't any source file for this server
if ( $null -ne $cfgSourceFileName ) {
# Only used for detailed log output, not in order to read the data from source file
...
# Testing of the key name of the source file, from destination file data set
# Does source file data variable contains proper key name of the source file, for coresponding destination file name ?
# if not then we know that it was not read from disc because the source file don't exist (Get-xData)
...
# Powershell Remoting, valid credentials for destination server need to be added to "Credential Manager"
...
# Set vartiable data that will be synchronized
$runspace.Open()
...
#endregion
#region Functions
function Convert-XLS { param( [parameter(Position=0,Mandatory=$true)]$XLS_FileName )
.\xls2csv.exe $XLS_FileName 2>&1 | Out-Null
}
function Get-Duplicate { param( [parameter(Position=0,Mandatory=$true)]$array )
	$hash = @{}
	$array | % { $hash[$_] = $hash[$_] + 1 }
	$hash.GetEnumerator() | ? { $_.value -gt 1 } | %{ $_.key }
}
function Get-xData { param([parameter(Position=0)]$dataCSV)
...
function Get-XLSFile { param( [parameter(Position=0)]$XLS_FileName )
if ( !$XLS_FileName ) { $XLS_FileName = ( Read-Host -Prompt "XLS Filename without extension" ) + ".xls" }
$XLS_FileName
}
function Get-Data { param( [parameter(Position=0,Mandatory=$true)]$XLS_FileName )
...
function Export-Log { param([parameter(Position=0)]$DataLogs, [parameter(Position=1)]$LogsFileName)
...
if ( $verbose ) { Start-ConfigurationDeploy ( Get-xData ( Get-Data ( Get-XLSFile $XLS_FileName ))) -verbose
